...
`gaps_filler` is a QGIS 3 plugin (per [`metadata.txt`](metadata.txt:8)) for processing hyperspectral drone frames with GPS/IMU integration into a single gap-filled orthomosaic. It started as a Plugin Builder skeleton and has grown into a Processing-framework provider that exposes six algorithms (filter / raw airborne georef / mosaic / fill / end-to-end pipeline / quality assessment).
...
| [`models.py`](models.py) | Passive path-bundling for hyperspectral flight lines. Public: [`FlightLineMeta`](models.py:15) frozen dataclass + [`discover_flight_line()`](models.py:28) (sibling `.hdr` required, `.times` / `.lcf` optional). No I/O, no header parsing. |
| [`airborne_georef.py`](airborne_georef.py) | Hyperspectral pushbroom raw-cube georeferencing. Supports: sidecar parsers ([`read_lcf()`](airborne_georef.py:78), [`read_times()`](airborne_georef.py:101), [`load_flight_line_poses()`](airborne_georef.py:155)); per-frame pose interpolation ([`interpolate_poses()`](airborne_georef.py:117)) aligned by **relative** time (`.lcf` and `.times` epochs differ); flat-earth ground grid ([`flat_ground_grid()`](airborne_georef.py:201)) intersecting per-pixel rays with a flat plane at user-supplied `ground_alt`; DEM-aware ground grid ([`dem_ground_grid()`](airborne_georef.py:541)) iteratively intersecting rays with terrain sampled from a DEM raster; flat / DEM GeoTIFF writer ([`write_flat_geotiff()`](airborne_georef.py:376) with optional `dem_path`) reading the raw `.bil` cube and writing a reprojected GeoTIFF via rasterio geolocation arrays + nearest-neighbor resampling. **Still no QGIS UI wiring** — the writer is callable from Python only. |
| [`airborne_georef_algorithm.py`](airborne_georef_algorithm.py) | [`AirborneGeorefAlgorithm`](airborne_georef_algorithm.py:1) — Stage Raw standalone (`gapsfiller:airborne_georef`); georeferences raw hyperspectral pushbroom cubes using sidecar LCF/times files. |
...
- Frame-open inversion in [`mosaic.mosaic_frames()`](mosaic.py:169) — chunk loop is now outer, band loop inner; each chunk opens its sources exactly once and iterates `rasterio.merge.merge(..., indexes=[b])`. Total open/close count drops from `band_count × n_chunks × chunk_size` to `n_chunks × chunk_size` (factor = `band_count`, значительное ускорение на гиперспектральных кубах). Progress now ticks per `(chunk, band)`.
...
- **New module [`models.py`](models.py:1)** — passive path-bundling for hyperspectral flight lines. Public: [`FlightLineMeta`](models.py:15) (frozen dataclass: `name`, `bil`, `hdr`, `times?`, `lcf?`) and [`discover_flight_line(bil_path)`](models.py:28) which finds sibling `.hdr` (required), `.times` / `.lcf` (optional). No I/O, no header parsing — that lives in [`envi_io.py`](envi_io.py:1).
...
- **Verified on sidecars** — example `.lcf` / `.times` parse and interpolate cleanly to 2000 frame poses; the projection grid has shape `(2000, 900)`, valid ratio `1.0`, lon ≈ `30.00058..30.00090`, lat ≈ `59.418675..59.418678`. End-to-end flat-earth raw `.bil` smoke test now succeeds (see 2026-05-07 Real hyperspectral camera 28 smoke test changelog below); DEM-aware mode still lacks a real DEM end-to-end test.
...
- **Still pending**: QGIS UI integration, real raw `.bil` end-to-end test **with a DEM** (flat-earth raw `.bil` smoke test landed — see 2026-05-07 Real hyperspectral camera 28 smoke test below), orientation calibration / sign flips if output is mirrored/rotated, optional bilinear DEM sampling later.

## Changelog — 2026-05-07 Real hyperspectral camera 28 smoke test
...
- **Input capture**: [`example/manual_hyperspectral_28.bil`](example/manual_hyperspectral_28.bil) with sidecars [`example/manual_hyperspectral_28.bil.hdr`](example/manual_hyperspectral_28.bil.hdr), [`example/manual_hyperspectral_28.bil.times`](example/manual_hyperspectral_28.bil.times), [`example/manual_hyperspectral_28.lcf`](example/manual_hyperspectral_28.lcf). Header: `900x2000`, `200` bands, `bil`, `uint16`, `200` wavelengths. `.times` row count matches `lines`; `.lcf` has `1604` rows and `12` columns.
...
- **End-to-end flat GeoTIFF succeeded**: wrote [`example/manual_hyperspectral_28_flat_smoke.tif`](example/manual_hyperspectral_28_flat_smoke.tif); readable GeoTIFF, size `900x9`, `200` bands, dtype `uint16`, CRS `EPSG:4326`, bounds `left=30.000590025604847`, `bottom=59.418678470247166`, `right=30.0009019613634`, `top=59.418681272733956`.
...
- **Acceptance signal**: New algorithm appears in Processing Toolbox → Hyperspectral gaps filler → Raster analysis; running it on a sample hyperspectral flight (cube + LCF + times, optionally DEM) produces a single georeferenced GeoTIFF at the chosen output path with the requested CRS.
...
- **Unit assumptions** LCF angles are in radians per hyperspectral camera spec; we convert boresight from degrees to radians before composing.
...
- ✅ done — QGIS-wrapper cosmetic cleanups: `handle_processing_exception` helper, canvas-styling attachment moved to [`canvas_styling.py`](canvas_styling.py:1), magic-number `280` replaced with `DEFAULT_HYPERSPECTRAL_BANDS` (see Recent changes below).
